define("dojox/mobile/TabBar", [
...
], function(array, declare, win, domClass, domConstruct, domGeometry, domStyle, domAttr, Contained, Container, WidgetBase, TabBarButton, has, window, BidiTabBar){
	...
	var TabBar =  declare(has("dojo-bidi") ? "dojox.mobile.NonBidiTabBar" : "dojox.mobile.TabBar", [WidgetBase, Container, Contained],{
		...
		//	alert ("size="+screen.availWidth+" w="+w+" dojo="+x.w);
			//w=screen.availWidth;
			w=x.w;
			...
			if(this._barType == "tabBar"){
				...
				if(this.fill == "always"){
					domClass.add(this.domNode, "mblTabBarFill");
				    	//alert("fill always");
					for(i = 0; i < arr.length; i++){
						arr[i].style.width = Math.floor(1*97/arr.length) + "%";
						arr[i].style.margin = "0";
			       	 //alert(arr[i].style.width+"-");
