...
function CreateNewViewlist(stat)
{
    ...
    if (newlistname == "")
    {
        myalert("Viewlist name cannot be blank");
        return;
    }
...
function ShowdeleteMyViewlist()
{
    newid = window.thislist['id'];
	dijit.registry.byId('viewlistMenu').hide();
	console.log("showdeletemyviewlist:" + newid);
...
function deleteMyViewlist(parm)
{
   // alert("deleting personal viewlist: " + 'my' + window.viewlistfordelete.substr(2));
...
//		alert("dojo for this "+dojo.byId(window.viewlistfordelete.toString()));
	...
	//alert("deleting viewlist for url"+url);
    dojo.io.script.get(
    {
        url: url,
        callbackParamName: "callback",
...
function RenameMyViewlist()
{
    ...
    var url = base + "user/renameMyViewlist?" + "email=" + window.email + "&listId=" + viewlistforrename + "&token=" + window.token + "&newname=" + newname;
    dojo.io.script.get(
    {
        url: url,
        callbackParamName: "callback",
...
function ShowRenameMyViewlist2()
{
    	dijit.registry.byId('viewlistMenu').hide();
    window.viewlistforrename = window.thislist['id'];
    vlname = window.thislist['name'];
    ...
    /* var playersDom = $('#MyViewListsEdit')[0];
    	for (var i=0; i<=playersDom.childElementCount; i++){
    		...
    		*/
    dojo.byId('RenameVLname').innerHTML = "'" + vlname + "'?";
    dijit.registry.byId('RenameViewlist').show();
}