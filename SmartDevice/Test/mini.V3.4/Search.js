...
/*	$("#searchSelectValue3").val("Fine Art");
	if (!domain)
	{
		...
		document.RadioForm.SearchTerm.value="Fine Art";
	}*/
	window.gettydomain="";
    dojo.empty(searchGrid);
	dojo.style('searchresults',"display","none");
...
//		$("#SearchBox").css("top", "41px");
	...
	//	dojo.style("SearchBox","height","180px");
		...
		else if (currCat=="Photography")
		{
			...
			dojo.byId('searchbox2').placeholder="Place, Photographer, Type";
		}
...
{
	window.GS=false;
	if (dojo.byId("radioGettySports").checked)
		showsearch("Getty-Sports");
	else if (dojo.byId("radioGettyNews").checked)
		showsearch("Getty-News");
	else if(dojo.byId("radioGettyEnt").checked)
		showsearch("Getty-Entertainment");
	else if(dojo.byId("radioPhotos").checked)
		showsearch("Photography");
	else
		showsearch();

	//console.log("getty?="+GS);
}	
