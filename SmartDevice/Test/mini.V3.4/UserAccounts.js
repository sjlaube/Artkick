...
function FBlogin()
{
	...
	gotoView(currentView,'blankview2');
	window.blankview2up=true;
//	alert("blankvie2 up")
    ...
    {}
	  if (!runInWebview)
		...
		{
		console.log("facebook login returns"+response['status']+' '+response['authResponse']['accessToken']);
		var url="http://mighty-mountain-4653.herokuapp.com/flogin/completeAuth?accessToken="+response['authResponse']['accessToken'];
      dojo.io.script.get(
        {
            url: url,
            callbackParamName: "callback",
            ...
            error: function(error)
            {
                ...
                {
                    alert("Network problem25b. Please check your connection and restart the app.");
                }
            },
            load: function(result)
            {
                ...
                if (result["Status"] == "success")
                {
					continueLogin(result['email'],result['token']);
				}
			}
		})
		},{scope:'email'});
...
//	alert("blankview2 up");
  ...
  var myParams = {
    'clientid' : '927829783635-lh1vje85dhdc99r7ji16r85jsl3ntso8.apps.googleusercontent.com', //You need to set client id
    'cookiepolicy' : 'single_host_origin',
    'callback' : 'GloginCallback', //callback function
    'approvalprompt':'force', //change to auto in production
...
function GloginCallback(result)
{
    ...
    { 
		...
		var url="http://mighty-mountain-4653.herokuapp.com/glogin/completeAuth?accessToken="+result['access_token'];
      dojo.io.script.get(
        {
            url: url,
            callbackParamName: "callback",
            ...
            error: function(error)
            {
                ...
                {
                    alert("Network problem25a. Please check your connection and restart the app.");
                }
            },
            load: function(result)
            {
...
// this is called when FB or Google login is successful
//alert("continueLogin with email="+email);
