class Client1Controller < ApplicationController
  ...
  def update2    
    ...
    begin     
      ...
      listSet = @db['viewlists'].find({"id"=>params[:list].to_s})
    end
    list = {}    
   ...
   if params[:players] == nil
    ...
    if imageSet.count > 0
      ...
      message['title'] = imageObj['Title']
      message['stretch'] = stretch
      message['nextPull'] = 0
      if imageObj["Artist Last N"]==nil
         message["caption"]=""
      else
         message["caption"]=imageObj["Artist First N"]+' '+imageObj["Artist Last N"]
      end
  ...
  def getSelectedPlayers
   ...
   def getUserStatus
    ...
    if user['curr_list'] == nil
      user['curr_list'] = defaultObj['viewlist'].to_i
    end
    
    if user['curr_cat'] == nil
      user['curr_cat'] = defaultObj['category']
    end
    
    if user['curr_image'] == nil
      user['curr_image'] = defaultObj['image'].to_i
    end
...
def getViewlist4
  ...
  def removeUser
    ...
    if(params[:id][0..2]=='top')
      ...
      @db["users"].update({"email"=>user["email"]},{"$set"=>{"curr_list_images"=>dynImages, "curr_list"=>viewlist["id"],
      "shuffle"=>params[:shuffle].to_i}})   
    ...
    images = viewlist["images"]
      ...
      if images == nil or images.length == 0
        defaultObj = @db['defaults'].find().to_a[0]
        listObj = @db['viewlists'].find({'id'=>defaultObj['viewlist'].to_i}).to_a[0]
        images = listObj['images']
        viewlist['images'] = listObj['images']
        viewlist['forward'] = 1
        viewlist['tarIndex'] = 1
        @db['users'].update({'email'=>user['email']},{'$set'=>{'curr_list'=>defaultObj['viewlist'].to_i,
