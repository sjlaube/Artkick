class Player1Controller < ApplicationController
  ...
  def getOwnedPlayers
    ...
    players = @db["clients"].find({"owner"=>userSet.to_a[0]["id"]},{:fields=>["account","nickname"]}).to_a
  ...
  def getPlayers
    ...
    playerHash.keys.each do |playerAccount|
      playerSet = @db["clients"].find({"account"=>playerAccount},{:fields=>["account","nickname","last_visit"]})
