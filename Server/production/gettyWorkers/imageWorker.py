...
def compressImage(db, bucket, id, url):
    ...
    db.gettyImages.update({'id':'getty'+id},{'$set':{'artkick_url':artkick_base+image_name, 'artkick_timestamp':utcMilli(), 'has_highres':True}})
    #threadLock.acquire()
    #global imageCount
    #imageCount += 1
    #threadLock.release()
...
def getStatus(db, firstTime):
    if firstTime == True:
       global imageCount
       imageCount = db.gettyImages.find({'has_highres':True}).count()

    peekSet = db.gettyImages.find({'has_highres':True}).limit(100).sort('refNum', pymongo.ASCENDING)
    
    global imagesToPeek
    imagesToPeek = Queue.Queue() #reset it first
    for image in peekSet:
       imagesToPeek.put(image['id'])

    print imageCount, imagesToPeek



def addImage(db, bucket, id):
    global imageCount
    global imagesToPeek
    global imageQueue
    ...
    if imageCount >= imageLimit:
        ...
        if imagesToPeek.qsize() == 0:
           getStatus(db, False)
        
        if imagesToPeek.qsize() > 0:
           rm_id = imagesToPeek.get()[5:]
           imageCount -= 1
           removeImage(db, bucket, rm_id)
           print 'peek', rm_id

    imageCount += 1
    imageQueue.put(id) #sync
...
def oneCycle(db, bucket):
    chunk = db.gettyToCheck.find()[0]
    print 'image num',len(chunk['images'])
    count = 0
    for id in chunk['images']:
        count += 1

        imageSet = db.gettyImages.find({'id':id})
        if imageSet.count() == 0:
            continue
        image = imageSet[0]
        if not ('has_highres' in image and image['has_highres'] == True):
            if ('refNum' in image and int(image['refNum']) > 0):
               print 'add image '+str(id[5:])
               addImage(db, bucket, id[5:])
        db.gettyToCheck.update({},{'$pull':{'images':id}})
...
def main():
    ...
    oauth()
    #ids = ['474227985', '474216085']
    #urls = getImageUrls(ids)
    
    #urlTest = urls[0]
    #compressImage(db, bucket, urlTest['ImageId'], urlTest['UrlAttachment'])
