...
client_key = 'vnnsf3q75b4ug6jrj7z3mrkq'
client_secret = 'EFkYn289vdEuDmpYJ8AjED42Uy66PEfajPKjxXxRBYt6G'
...
class urlWorker(threading.Thread):
    ...
    def run(self):
        print "Starting "+self.name
        while True:
            print 'url thread running...'
            global urlWorkerStamp
            urlWorkerStamp = int(round(time.time() * 1000))
            cart = []
            global imageQueue
            global urlQueue
...
# peek 100 images first
    ...
    #check url worker
    print 'checking urlWoker'
    currTime = int(round(time.time() * 1000))
    global urlWorkerStamp
    if urlWorkerStamp > -1 and currTime - urlWorkerStamp > 2*60*1000:
       print 'urlWorker is gone... restart it...'
    ...
    global imageQueue
    print 'expire', expire
    imagesToDevSet = db.gettyImages.find({'has_highres':False, 'refNum':{'$gt':0}, 'processing':False, 'photos':{'$exists':False}, 'last_visit':{'$gte':utcMilli()-expire}}).limit(100).sort('last_visit', pymongo.DESCENDING)
...
def main():
    ...
    db.gettyImages.update({'photos':{'$exits': False}},{'$set':{'processing':False}}, multi=True)
